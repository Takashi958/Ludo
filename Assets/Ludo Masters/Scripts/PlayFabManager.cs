...
public class PlayFabManager : Photon.PunBehaviour, IChatClientListener
{
    ...
    public void getPlayerDataRequest()
    {
        ...
        PlayFabClientAPI.GetUserData(getdatarequest, (result) =>
        {
            ...
            if (result.Data == null || !result.Data.ContainsKey("PremiumAccount"))
            {
                Debug.Log("No Premium");
            }
            else
            {
                Debug.Log("Premium account: " + result.Data["PremiumAccount"].Value);
                premiumAccountkey = result.Data["PremiumAccount"].Value;
                IsPremium();
            }
    ...
    {
        Debug.Log("Error User Data Send");
    }
